...
class TestClassifyKeywordsAsBranded:
    ...
    def test_broad_match_includes_substrings(self, sample_df):
        # Broad match: "cable" matches anywhere it appears as a substring
        ...
        # "cablecar" contains "cable" → broad match hits
        assert result.loc[result["keyword"] == "cablecar", "is_branded"].iloc[0]
        # "excable shoes" also contains "cable" → broad match hits
        df2 = pd.DataFrame({"keyword": ["excable shoes", "cable shoes"]})
        result2 = classify_keywords_as_branded(df2, ["cable"])
        assert result2.loc[result2["keyword"] == "excable shoes", "is_branded"].iloc[0]
    ...
    def test_classify_broad_match_includes_substring(self):
        # Broad match: all three contain "cable" → all branded
