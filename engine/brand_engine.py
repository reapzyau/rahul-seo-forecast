...
Broad match: a keyword is branded if it contains any brand term as a substring.
"""
...
def _broad_match_pattern(term: str) -> re.Pattern:
    """Build a case-insensitive broad-match regex for a brand term."""
...
def classify_keywords_as_branded(
    ...
    Matching is case-insensitive broad match — a keyword is branded if the
    brand term appears anywhere in the keyword string.
    ...
    patterns = [_broad_match_pattern(t) for t in brand_terms if t.strip()]

    def _is_branded(keyword: str) -> bool:
        return any(p.search(keyword) for p in patterns)
